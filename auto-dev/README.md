...
- ✅ **GitHub 통합**: 이슈 번호로 직접 작업 시작 (Phase 3 ✨)
- ✅ **자동 PR 생성**: 작업 완료 시 자동으로 Pull Request 생성 (Phase 3 ✨)
...
│   └── pr-manager.md        # Pull Request 자동 생성
├── commands/
...
#### 5. PR Manager Agent (Phase 3 ✨)

작업 완료 후 Pull Request를 자동으로 생성하는 에이전트입니다.

**역할**:
- Git 히스토리 분석 및 변경사항 파악
- PR 제목 자동 생성 (feat/fix/refactor)
- 포괄적인 PR 본문 작성 (요약, 변경사항, 테스트 정보)
- GitHub 이슈 자동 연결 (Closes #123)
- `gh` CLI를 통한 PR 생성

**자동 실행 조건**:
- GitHub 이슈 번호로 작업이 시작된 경우 (`/auto-dev "#123"`)
- 작업이 성공적으로 완료된 경우
...
## Summary
- OAuth 2.0 인증 플로우 구현
- Google 및 GitHub 제공자 지원
- 사용자 세션 관리 추가

## Changes
- **Added**:
  - src/auth/oauth-service.ts - OAuth 플로우
  - src/auth/providers/* - 제공자 구현

- **Modified**:
  - src/routes/auth.ts - OAuth 엔드포인트

## Testing
- [x] pr-review-toolkit 에이전트 리뷰 완료
- [x] 모든 critical 이슈 해결

Closes #123

🤖 Generated by Auto-Dev Plugin
```
...
# GitHub 이슈 (단일) - Phase 3
/auto-dev "#123"

# GitHub 이슈 (여러 개) - Phase 3
...
### Phase 3 (v0.3.0) - GitHub 통합 ✅

- ✅ **GitHub 통합**: 이슈 번호로 직접 작업 (`/auto-dev "#123"`)
- ✅ **자동 PR 생성**: 작업 완료 시 gh pr create 자동 실행
- ✅ **이슈 자동 연결**: PR에 "Closes #123" 자동 추가

### Phase 4 (향후 계획)

- ⚡ **CI 모니터링**: PR 체크 대기 및 결과 보고
- ⚡ **자동 재작업**: CI 실패 시 자동으로 수정 시도
- ⚡ **PR 리뷰 통합**: 코드 리뷰 코멘트 자동 반영

## 필요 조건

- **Claude Code**: 최신 버전
- **Git**: Worktree 지원 (Git 2.5+)
- **GitHub CLI (gh)**: GitHub 통합 및 PR 생성용 (Phase 3+)
...
0.3.0 (Phase 3 - GitHub 통합 및 자동 PR 생성)
